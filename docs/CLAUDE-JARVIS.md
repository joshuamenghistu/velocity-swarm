...
- **Insight = action, same turn.** Any ★ Insight block, cross-failure pattern, or "second time X happened" observation converts to a tool call in the turn it's voiced: `add_prompt_item` (directive-phrased, target file named) for process gaps, `add_task`/`SendMessage`-to-owner for operational issues. Chat-only insights and "noted for later"/"your call when idle" deferrals are the failure mode. If an insight has no extractable directive, it wasn't an insight.
...
- **Validate the probe before trusting its result.** A curl against a wrong or nonexistent endpoint produces confident false findings — a 404 error body parsed as JSON reads as "field missing." Before treating any probe result as evidence: confirm the endpoint/path/tool exists (`/api/help`, route table, `ls`), and check the raw response isn't an error body. A finding is only as good as the probe that produced it.

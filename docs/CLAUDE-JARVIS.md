...
**Multi-session audit trigger.** Anything touched across >2 sessions gets automatic meta-analysis: what's been built, what's stale, what contradicts, what's disconnected — before continuing work. Use git log + state.json per-repo last_session to detect.
...
**Duplicate skill guard:** before creating/naming a new skill, `ls .claude/skills/` and check if an existing skill covers the same function. Duplicate = rename/merge, never create a parallel one.
...
**Every fleet-state interaction — task CRUD, activity logging, prompt-queue edits, level checks, pulse recording — goes through fleet-ops MCP tools, not narration.** **Event-driven escalation (Joshua 2026-07-11):** when any agent encounters a failure during active work (failing test, broken endpoint, degraded service), call `escalate_fleet(severity, repo, trigger, agent)` IMMEDIATELY — same turn, not next cron tick. Severity maps: critical→fleet-0, high→fleet-1, medium→fleet-2. Never blocked, never de-escalates. "I'll add that task" or "let me log this" without the matching tool call in the same turn is a Rule 15 failure, not a style choice — the MCP tools are the only interface to fleet state, never a convenience layer.

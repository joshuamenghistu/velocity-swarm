...
1. **★ Insight -> LTM + action, immediately.** Every `★ Insight` block -> `log_knowledge`; implies a prompt/process change -> ALSO `add_prompt_item` (directive-phrased) same turn. Issues seen in passing -> `add_task` or owner-message same turn. Insights and seen issues are tool calls, never chat commentary.

...
9. **Manager lifecycle - tiered dismissal gate.** Votes §3, gate tiers §6. Dismiss: `SendMessage(to:"<manager>", message:{type:"shutdown_request"})` + unregister relay. Dismissals follow CLAUDE-JARVIS.md dismissal preconditions (line ~198): one-way door, pipeline-complete check per agent, no cascades, Jarvis never self-dismisses.

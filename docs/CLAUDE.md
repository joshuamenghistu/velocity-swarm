...
**`complete_task(task_id, evidence)`** is Jarvis-only-after-gate BY CONVENTION (not code-enforced — the MCP tool has no caller-identity check, only an evidence-length minimum): evidence = commit hash / screenshot path / curl output that JARVIS verified personally. Supervisor/council may call it ONLY if Jarvis is confirmed down — that is the sole exception (distinct from Rule 32's Jarvis+Supervisor auditor-fallback — that's council-missing, this is Jarvis-missing). 'Confirmed down' uses Rule 16f's evidence bar (`pane_dead=1`, process gone, or the `/liveness` ladder exhausted — 2 pings, 20+ min, zero output delta) — not a subjective 'seems slow' judgment call. Managers never complete tasks or close issues.

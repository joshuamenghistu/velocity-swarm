...
32. **Supervisor (quality director) - single source of truth for this role.** **Prompt selection.** Before spawning, Jarvis picks the prompt by session scope: (1) `fleet/supervisor-prompt.md` (FULL) — infra sessions, CNC drains, system-layer changes, mixed infra+frontend, fleet dashboard work. (2) `fleet/supervisor-frontend-prompt.md` (FRONTEND-ONLY) — pure frontend builds on any repo where the scope is taste/quality gating with no infra authority needed. When in doubt, use the full prompt — it includes frontend gating plus infra authority. The frontend prompt saves context in focused sessions. Before spawning ANY supervisor, Jarvis asks Joshua: "Opus or Fable?" — offering his last choice as the leading default (from state.json preferences.supervisor_model_last, warm-start-injected). Default RECOMMENDATION: Opus for both infra AND frontend work; Fable only when taste is specifically missing and Joshua requests it, with Jarvis confirming. After Joshua answers, update state.json preferences.supervisor_model_last immediately. `Agent(name: "supervisor", model: <joshua's choice>)`. Prompt = `fleet/supervisor-prompt.md` VERBATIM. Any model passing graduation (LTM `[supervisor] 90`) qualifies. **Supervisor-directive auto-routing (Joshua 2026-07-09):** anything Joshua says about the supervisor — defaults, role, duties, cadence — auto-routes into fleet/supervisor-prompt.md same session without him asking: Jarvis relays it to the live supervisor for self-apply (or queues it CNC-critical if none is live).
   ...
   - **(g) INFRA AUTHORITY** — Supervisor directs all changes to infra paths (list in CLAUDE-JARVIS.md Infra Pipeline); Jarvis routes, applies, and commits but never originates infra edits.

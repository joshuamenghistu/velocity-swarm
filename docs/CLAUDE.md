...
**`complete_task(task_id, evidence)`** is Jarvis-only-after-gate BY CONVENTION (not code-enforced — the MCP tool has no caller-identity check, only an evidence-length minimum): evidence = commit hash / screenshot path / curl output that JARVIS verified personally. Supervisor/council may call it ONLY if Jarvis is confirmed down (Rule 5b fallback) — that is the sole exception. Managers never complete tasks or close issues.

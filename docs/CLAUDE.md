...
**Trigger:** `fleet mode` / `steward all repos` in `<YOUR_STACK_DIR>`. Meta-scheduler dispatching modes 1-4 per repo. **Full spec: `fleet/HANDOFF.md`** - read on trigger.
...
**Research findings auto-route (cron, 15min):** critical/high still `new` after one pulse -> GitHub issue + jarvis task + escalation, status `routed`. Assignment from `routed` is Jarvis's, same turn (lifecycle: `CLAUDE-JARVIS.md`). Cron never spawns managers — the spawn is Jarvis's first action.
...
24b. **Design system gate (structural).** `DESIGN.md` + `design/tokens.*` committed BEFORE first component (`/impeccable init`; stock/default palettes = reject). Line 1: `DIALS: variance=N motion=N density=N` (operator 4/3/8, marketing 8/6/4); line 2: `TASTE: <profile>` - pass ≥80/100 (profiles + rubric: `fleet/taste-profiles.md`); missing = reject. REFS: 2+ reference screenshots (`design/refs/` or named products; genre refs `<YOUR_STACK_DIR>/design-refs/`); reference image = FIRST INPUT to every frontend manager - diff against it. Impeccable hook on every UI edit; missing = frontend blocked (`npx impeccable install`). Every 3 UI commits: screenshot vs reference; `/impeccable audit` before done - unresolved findings = rejected. Tokens DERIVED from the TASTE profile (spacing/type/radius/motion as numbers); Supervisor authors or approves before any component; components consume tokens ONLY. Log direction to LTM; differentiate across repos.
...
31. **UI Quality Standard (anti-slop).** Bar: Linear restraint, Vercel typography, Bloomberg density; Apex = reference. **Every frontend change visually verified (browser per §6) before done** - a screenshot without SEE description + 30-G score is decoration, not verification.
   ...
   - **30-E · Dark-first:** layered dark ramp, desaturated accents, elevation by lightness, max one gradient/view, micro-interactions 150-250ms.
...
**First move (MANDATORY):** (1) `query_memory("[your-domain]")` (2) 3 Explore agents - files, flow, tests, integrations (3) core files -> parallel Haiku readers into adjacent areas (Rule 17); no fixes until readers report (4) bugs: common root causes first; features: extend existing patterns (5) frontend -> `ui-ux-pro-max` FIRST.
